										...
										default:
										{
											...
											switch (lpSendUserData->DefMsg.wIdent)
											{
												...
												case DB_SAVEHUMANRCD:
												{
													...
													fnDecode6BitBufA((pszDivide + DEFBLOCKSIZE + HUMLOADRDCVLOCKSIZE), (char *)lpSendUserData->lpbtAddData, sizeof(_THUMANRCD));

													int nRemainLen = (int)(pszEnd - pszFirst) - (DEFBLOCKSIZE + HUMLOADRDCVLOCKSIZE + HUMANRCDBLOCKSIZE);

													lpSendUserData->lpbtAddData2 = new BYTE[nRemainLen + 1];

													memcpy(lpSendUserData->lpbtAddData2, (pszDivide + DEFBLOCKSIZE + HUMLOADRDCVLOCKSIZE + HUMANRCDBLOCKSIZE), nRemainLen);
