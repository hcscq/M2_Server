...
*/	
	//_LPTCLIENTITEMRCD lpClientItemRcd = (_LPTCLIENTITEMRCD)lpClientItemRcd;
	memcpy(&lpClientItemRcd->tUserItemAbility, lpUserItemRcd, sizeof(_TUSERITEMRCD));
	switch (btType)
	{
		case 0:
		{
			...
			lpClientItemRcd->wSC		= MAKEWORD(wSC, wSC2 + lpUserItemRcd->btValue[2]); //3:행운, 4:저주
			lpClientItemRcd->wAC		= MAKEWORD(wAC + lpUserItemRcd->btValue[3], wAC2 + lpUserItemRcd->btValue[5]);  //정확
			lpClientItemRcd->wMAC		= MAKEWORD(wMAC + lpUserItemRcd->btValue[4], wMAC2 + lpUserItemRcd->btValue[6]);  //공격속도(-/+)
