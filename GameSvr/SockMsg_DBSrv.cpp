	...
	switch (WSAGETSELECTEVENT(lParam))
	{
		case FD_CONNECT:
		{
			if (CheckSocketError(lParam))
			{
				...
				InsertLogMsg(_TEXT("data svr connected."));
...
//					SetThreadPriority((HANDLE)hThreadForMsg, THREAD_PRIORITY_HIGHEST);
				...
				if(InitServerSocket(g_ssock, &g_saddr, _IDM_SERVERSOCK_MSG, nPort, 1)){//打开端口 5000
					//InsertLogMsg(IDS_STARTSERVICE);
					TCHAR	szSrvPort[16];
					_stprintf(szSrvPort, _T(":%d"),nPort);
					InsertLogMsgParam(IDS_STARTSERVICE, szSrvPort, LOGPARAM_STR);
				}
