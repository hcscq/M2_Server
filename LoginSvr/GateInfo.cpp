...
void CGateInfo::MakeNewUser(char *pszPacket)
{
	...
	if (DefMsg.wIdent == CM_ADDNEWUSER)
	{
		...
		*pszPassword = '\0';
		...
		{
			char szQuery[1024];
			sprintf( szQuery, //done 2012 /6/29
				"INSERT TBL_ACCOUNT( FLD_LOGINID, FLD_PASSWORD, FLD_USERNAME, FLD_CERTIFICATION ) "//certification
				"VALUES( '%s', '%s', '%s', 0 )",
...
void CGateInfo::ReceiveClientInfo(int nSocket)
{
...
void CGateInfo::ProcAddUser(SOCKET s, char *pszData)
{
	...
	szEntryInfo[len] = '\0';
	//char				szEntryInfoa[2048];
	//pszData+=23;
	//for(int i=20;i<50;i++){
	//	fnDecode6BitBufA(pszData++, (char *)&szEntryInfoa, sizeof(szEntryInfo));
	//	int aa;
	//}
