...
UINT WINAPI LoadAccountRecords(LPVOID lpParameter)
{
	...
	if ( pRec->Execute( "SELECT * FROM TBL_SERVERINFO WHERE FLD_TYPE=0" ) )
	{
		while ( pRec->Fetch() )
		{			
	...
	int nPort=5500;
	if (InitServerSocket(g_gcSock, &g_gcAddr, _IDM_GATECOMMSOCK_MSG, nPort, 1)){
		SwitchMenuItem(TRUE);
		InsertLogMsgParam(IDS_OPEN_PORT,&nPort,LOGPARAM_INT);
	}
...
void OnCommand(WPARAM wParam, LPARAM lParam)
{
	switch (LOWORD(wParam))
	{
		...
		{
			if(GetDBManager()->m_pConn==NULL){ MessageBox(NULL,_T("数据库连接不可用，不能启动服务！"),_T("登陆服务器"),MB_OK);return;}
			g_fTerminated = FALSE;
